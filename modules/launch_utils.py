...
def run_pip(command: str, desc=None, live=default_command_live):
    if args.skip_install:
        return

    index_url_line = f"--index-url {index_url}" if index_url != "" else ""
    return run(
        f"uv pip {command} --prefer-binary {index_url_line}",
...
def run_git(
    dir, name, command, desc=None, errdesc=None, custom_env=None, live: bool = default_command_live, autofix=True
):
    try:
...
def git_clone(url, dir, name, commithash=None):
    ...
    if os.path.exists(dir):
        ...
        current_hash = run_git(
            dir, name, "rev-parse HEAD", None, f"Couldn't determine {name}'s hash: {commithash}", live=False
        ).strip()
        ...
        if (
            run_git(
                dir, name, "config --get remote.origin.url", None, f"Couldn't determine {name}'s origin URL", live=False
            ).strip()
            != url
        ):
...
def prepare_environment():
    ...
    torch_command = os.environ.get(
        "TORCH_COMMAND", f"uv pip install torch==2.1.2 torchvision==0.16.2 --extra-index-url {torch_index_url}"
    )
    if args.use_ipex:
